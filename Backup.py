...
class FrameWriterThread(QThread):
    ...
    def run(self):
        while self.running:
            try:
                burst_index, frames = self.queue.get(timeout=1)
                if frames and len(frames) > 0:
                    save_path = os.path.join(self.save_dir, f"burst_{int(burst_index):03d}.tif")
                    with tifffile.TiffWriter(save_path) as tif:
                        for f in frames:
                            tif.save(f.astype(np.uint16))
                    self.finished_signal.emit(f"Saved burst_{int(burst_index):03d}.tif")
                self.queue.task_done()
                self.log_event_signal.emit(f"Saved {self.save_dir} ({len(frames)} frames)", "green")
            except queue.Empty:
                continue
...
class BurstThread(QThread):
    ...
    def __init__(self, burst_index, duration_s, core):
        super().__init__()
    ...
    def run(self):
        ...
        while (time.time() - start_time) < self.duration_s and not self._stop_event.is_set():
            if self.core and self.core.getRemainingImageCount() > 0:
                img = self.core.popNextImage()
                frame = np.array(img, dtype=np.uint16)
                self.frames_array.append(frame)
            time.sleep(0.001)  # just wait; frames are collected via signal

        self.burst_done.emit(self.burst_index, self.frames_array)
...
class LiveImagingGUI(QWidget):
    ...
    def start_experiment(self):
        ...
        if not save_path:
            self.log_event("Please select a save folder before starting experiment", "red")
            return

    # Stop any previous experiment thread
        if getattr(self, "experiment_thread", None) and self.experiment_thread.isRunning():
            self.experiment_thread.experiment_running = False
            self.experiment_thread.wait()

    # Create new experiment thread
        self.experiment_thread = ExperimentThread(
            ...
            save_path=save_path,
            total_bursts=int(self.total_time_spin.value() * int(self.fps_combo.currentText()) / self.burst_duration_spin.value()),  # example
            burst_duration_s=self.burst_duration_spin.value(),
            pause_between_bursts_s=self.wait_interval_spin.value(),
            ttl_delay_ms=int(self.trigger_time_spin.value()),
            ttl_freq=int(self.ttl_frequency_spin.value()),
            ttl_duration_ms=int(self.ttl_duration_spin.value()),
            ttl_mode=self.ttl_mode_combo.currentText()
        )

        self.experiment_thread.log_event_signal.connect(self.log_event)
        # self.experiment_thread.burst_started.connect(self.on_burst_started)
        # self.experiment_thread.burst_done.connect(self.on_burst_done)
        self.experiment_thread.experiment_finished.connect(lambda: self.set_overlay("READY", color="green"))

        self.experiment_thread.start()
        self.log_event("Experiment started", "green")

    def stop_experiment(self):
        if getattr(self, "experiment_thread", None) and self.experiment_thread.isRunning():
            self.experiment_thread.experiment_running = False  # signal thread to stop
            self.experiment_thread.wait()  # wait for clean exit
            self.log_event("Experiment stopped", "orange")
            self.set_overlay("STOPPED", color="orange")
        else:
            self.log_event("No experiment running to stop", "red")
    ...
    def on_core_loaded(self, success, core):
        ...
        self.core.setROI(0, 0, 1200, 1200)
...
class ExperimentThread(QThread):
    burst_started = pyqtSignal(int)
    burst_done = pyqtSignal(int, object)
    experiment_finished = pyqtSignal()
    log_event_signal = pyqtSignal(str, str)

    def __init__(self, gui, arduino, core, live_thread, save_path, total_bursts,
                 burst_duration_s, pause_between_bursts_s, ttl_delay_ms, ttl_freq, ttl_duration_ms, ttl_mode):
        super().__init__()
        self.gui = gui  # reference to GUI for logging/overlay
        ...
        self.pause_between_bursts_s = pause_between_bursts_s
        self.ttl_delay_ms = ttl_delay_ms
        self.ttl_freq = ttl_freq
    ...
    def run(self):
        self.gui.set_overlay("EXPERIMENT IN PROGRESS...", color="blue")
        self.gui.log_event("Experiment started", "green")

        # Start writer thread
        self.writer_thread = FrameWriterThread(self.burst_job_queue, self.save_path)
        self.writer_thread.log_event_signal.connect(self.gui.log_event)
        self.writer_thread.start()

        while self.experiment_running and self.burst_index < self.total_bursts:
            self.burst_index += 1
            burst_num = self.burst_index
            self.gui.log_event(f"Burst {burst_num} scheduled (TTL in {self.ttl_delay_ms} ms)", "orange")
            self.burst_started.emit(burst_num)

            # Start burst thread
            burst_thread = BurstThread(burst_index=burst_num, duration_s=self.burst_duration_s, core=self.core)
            self.live_thread.new_frame.connect(burst_thread.collect_frame)
            burst_thread.burst_started.connect(self.burst_started)
            burst_thread.burst_done.connect(self.burst_done)
            burst_thread.log_event_signal.connect(self.gui.log_event)
            burst_thread.start()

            # Schedule TTL independently
            time.sleep(self.ttl_delay_ms / 1000.0)
            self.send_ttl()

            # Wait until burst finishes
            burst_thread.wait()
            frames_array = burst_thread.frames_array
            save_folder = self.save_path
            os.makedirs(save_folder, exist_ok=True)
            out_path = os.path.join(save_folder, f"burst_{burst_num:03d}.tif")
            self.burst_job_queue.put((out_path, frames_array))
            self.gui.log_event(f"Burst {burst_num} saved to {out_path}", "green")

            # Pause between bursts
            if self.burst_index < self.total_bursts:
                time.sleep(self.pause_between_bursts_s)

        self.experiment_running = False
        self.gui.set_overlay("EXPERIMENT STOPPED", color="red")
        self.gui.log_event("All bursts completed", "orange")
        self.writer_thread.stop()
        self.experiment_finished.emit()

    def send_ttl(self):
        if not self.arduino:
            self.gui.log_event("Arduino not connected, TTL skipped", "red")
            return
        self.arduino.write(b'1')
        time.sleep(0.005)
        self.arduino.write(b'0')
        if self.ttl_mode == "Train:":
            interval = 1.0 / self.ttl_freq
            pulses = int(self.ttl_duration_ms / 1000 * self.ttl_freq)
            for _ in range(pulses):
                self.arduino.write(b'1')
                time.sleep(0.001)  # pulse width 1 ms
                self.arduino.write(b'0')
                time.sleep(interval - 0.001)
            self.gui.log_event(f"TTL pulse sent (mode={self.ttl_mode}, freq={self.ttl_freq} Hz, duration={self.ttl_duration_ms} ms)", "blue")
        if self.ttl_mode == "Single Pulse":
            self.gui.log_event(f"TTL pulse sent (mode={self.ttl_mode})", "blue")
# -------------------- Main --------------------
